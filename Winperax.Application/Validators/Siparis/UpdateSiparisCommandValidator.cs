    ...
    public class UpdateSiparisCommandValidator : AbstractValidator<UpdateSiparisCommand>
    {
        public UpdateSiparisCommandValidator()
        {
            RuleFor(x => x.Id)
                ...
                .Length(1, 50).WithMessage("Sipariş ID 1 ile 50 karakter arasında olmalıdır.");

            RuleFor(x => x.SiparisNo)
                .NotEmpty().WithMessage("Sipariş numarası boş olamaz.")
                .MaximumLength(50).WithMessage("Sipariş numarası en fazla 50 karakter olabilir.");
            ...
            RuleFor(x => x.SiparisTarihi)
                ...
                .LessThanOrEqualTo(DateTime.Now.AddDays(1)).WithMessage("Sipariş tarihi gelecekte olamaz.");

            RuleFor(x => x.TeslimTarihi)
                .GreaterThanOrEqualTo(x => x.SiparisTarihi).WithMessage("Teslim tarihi sipariş tarihinden önce olamaz.")
                .LessThanOrEqualTo(DateTime.Now.AddMonths(6)).WithMessage("Teslim tarihi sipariş tarihinden 6 aydan fazla ileride olamaz.");

            RuleFor(x => x.Durum)
                .NotEmpty().WithMessage("Sipariş durumu boş olamaz.")
                .MaximumLength(20).WithMessage("Sipariş durumu en fazla 20 karakter olabilir.");
        }
    }
}